...
class RelationshipRepository:
    ...
    async def create(self, relationship_create: RelationshipCreate) -> Relationship:
    ...
    async def get_by_id(self, relationship_id: UUID) -> Optional[Relationship]:
    ...
    async def get_by_source_and_target(self, source_id: UUID, target_id: UUID, rel_type: str) -> Optional[Relationship]:
    ...
    async def get_by_source_asset(self, source_id: UUID, page: int = 1, size: int = 50) -> Tuple[List[Relationship], int]:
        stmt = select(Relationship).options(
            selectinload(Relationship.target_asset)
    ...
    async def get_by_target_asset(self, target_id: UUID, page: int = 1, size: int = 50) -> Tuple[List[Relationship], int]:
        stmt = select(Relationship).options(
            selectinload(Relationship.source_asset)
