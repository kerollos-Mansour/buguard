...
class AssetService:
    ...
    def create_asset(self, asset_in: AssetCreate) -> Asset:
        ...
        if existing_asset:
            raise DuplicateAssetException(
                f"Asset with type '{asset_in.type}' and value '{asset_in.value}' already exists."
            )
        ...
        if asset_in.type is not None or asset_in.value is not None:
            ...
            if new_type != asset.type or new_value != asset.value:
                ...
                if existing and existing.id != asset_id:
                    raise DuplicateAssetException("Another asset with this type and value already exists.")
    ...
    def add_tag_to_asset(self, asset_id: UUID, tag_name: str) -> Asset:
        ...
        if not tag:
            raise TagNotFoundException(f"Tag '{tag_name}' not found.")
