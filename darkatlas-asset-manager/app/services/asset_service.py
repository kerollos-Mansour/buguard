...
class AssetService:
    ...
    async def create_asset(self, asset_in: AssetCreate) -> Asset:
        existing_asset = await self.asset_repository.get_by_type_and_value(asset_in.type, asset_in.value)
        if existing_asset:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Asset with type '{asset_in.type}' and value '{asset_in.value}' already exists."
            )
        return await self.asset_repository.create(asset_in)
    ...
    async def get_assets(self, page: int = 1, size: int = 50, filters: Optional[Dict[str, Any]] = None, search: Optional[str] = None) -> Tuple[List[Asset], int]:
        return await self.asset_repository.get_all(page=page, size=size, filters=filters, search=search)

    async def update_asset(self, asset_id: UUID, asset_in: AssetUpdate) -> Asset:
        ...
        if asset_in.type is not None or asset_in.value is not None:
            ...
            if new_type != asset.type or new_value != asset.value:
                existing = await self.asset_repository.get_by_type_and_value(new_type, new_value)
    ...
    async def add_tag_to_asset(self, asset_id: UUID, tag_name: str) -> Asset:
        asset = await self.get_asset(asset_id)
        ...
        await self.asset_repository.add_tag(asset, tag)
        return asset

    async def remove_tag_from_asset(self, asset_id: UUID, tag_name: str) -> Asset:
        asset = await self.get_asset(asset_id)
        tag = await self.tag_repository.get_by_name(tag_name)
        if not tag:
            raise HTTPException(
        ...
        await self.asset_repository.remove_tag(asset, tag)
        return asset
