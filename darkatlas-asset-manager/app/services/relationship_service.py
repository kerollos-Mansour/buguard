...
class RelationshipService:
    ...
    def create_relationship(self, relationship_in: RelationshipCreate) -> Relationship:
        ...
        if not source_asset:
            raise AssetNotFoundException("Source asset not found.")

        target_asset = self.asset_repository.get_by_id(relationship_in.target_asset_id)
        if not target_asset:
            raise AssetNotFoundException("Target asset not found.")

        if source_asset.id == target_asset.id:
            raise RelationshipException("Asset cannot be related to itself.")
